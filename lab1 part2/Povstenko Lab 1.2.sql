...
SELECT MAX(QUOTEDPRICE) FROM ORDER_DETAILS

--3.Клієнтів з якого міста в компанії найбільше?
SELECT TOP 1 EMPCITY FROM EMPLOYEES
GROUP BY EMPCITY
ORDER BY COUNT(EMPCITY) DESC

--4.Який офіс (з якого міста) дає компанії найбільші прибутки?
SELECT TOP 1 VENDNAME, VENDCITY, RETAILPRICE - WHOLESALEPRICE AS PROFIT
FROM PRODUCTS AS P
...
SELECT TOP 1 PRODUCTNUMBER, SUM(QUANTITYORDERED) AS SUM_QUANTITYORDERED
FROM ORDER_DETAILS
...
SELECT TOP 1 *, RETAILPRICE - WHOLESALEPRICE AS PROFIT
FROM PRODUCTS AS P
INNER JOIN PRODUCT_VENDORS AS PV
ON P.PRODUCTNUMBER = PV.PRODUCTNUMBER
